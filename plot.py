...
def generate_svg_plot(dictionary, min_, max_, liste_cord, width, liste):
    ...
    for index in range(len(lx)-2):
        plt.annotate(dictionary['points'][index], xy=(lx[index], ly[index]), xytext=(+10, -10),
...
def generate_svg_plot2(dictionary, min, max, liste_cord, width, choice):
    ...
    for index in range(len(lx)-1):
        plt.annotate(index+1, xy=(lx[index], ly[index]), xytext=(+10, -10),
                     textcoords='offset points', horizontalalignment='center', verticalalignment='center')
    ...
    if min >= 0:
        for func in dictionary.keys():
            if func == 'exp':
                ...
                b = dictionary[func]['b']
                if choice == 'linear':
                    plt.plot(x, funclin(x, a, b), '#D9B504',
                             label="Lin Fitted Curve")
        ...
        for func in dictionary.keys():
            if func == 'exp':
                ...
                b = dictionary[func]['b']
                if choice == 'linear':
                    plt.plot(x, funclin(xneg, a, b), '#D9B504',
                             label="Lin Fitted Curve")
...
def pie_chart(names, probas):
    ...
    if len(names) == 3:
        plt.figure(1)
        plt.subplot(121)
        plt.pie([1], labels=[names[0]], colors=colors, startangle=90,
                autopct='%1.0f%%', textprops={"weight": "bold"})
        plt.title("A", fontdict={"fontweight": "bold"})
        plt.subplot(122)
        plt.pie(probas, labels=names[1:], colors=colors, startangle=90,
                autopct='%1.0f%%', textprops={"weight": "bold"})
        plt.title("B", fontdict={"fontweight": "bold"})
        fig = plt.gcf()
        fig.set_size_inches(8, 4)
    else:
        plt.pie(probas, labels=names, colors=colors, startangle=90,
                autopct='%1.0f%%', textprops={"weight": "bold"})
        fig = plt.gcf()
