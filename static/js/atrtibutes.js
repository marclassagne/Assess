...
$("#quantiname").append(
    '<input type="text" class="form-control" id="att_name_quanti" placeholder="Name" spellcheck="false" onkeypress="verifierCaracteres(event); return false;" autocomplete="off" pattern="[a-z]{4,8}" />'
);
$("#quantiname").append(
    '<p style="font-size:10px;"> Use only alphanumerical chars (non case sensitive)</p>'
);
$("#qualiname").append(
    '<input type="text" class="form-control" id="att_name_quali" placeholder="Name" spellcheck="false" onkeypress="verifierCaracteres(event); return false;" autocomplete="off"  />'
);
$("#qualiname").append(
    '<p style="font-size:10px;"> Use only alphanumerical chars (non case sensitive)</p>'
);
...
add_value_med.addEventListener("click", function () {
    ...
    new_item.innerHTML =
        "<input type='text' class='form-control' id='att_value_med_quali_" +
        String(longueur + 1) +
        "' placeholder='Intermediary Value " +
        String(longueur + 1) +
        "'/>";
...
del_value_med.addEventListener("click", function () {
    ...
    if (longueur != 1) {
        ...
        alert(
            "Please put at least one medium value for the attribute " +
                $("#att_name").val()
        );
    }
});
...
$(function () {
    ...
    $(".del_simu").click(function () {
        if (
            confirm(
                "You are about to delete all the attributes and their assessments.\nAre you sure ?"
            ) == false
        ) {
            return;
        }
    ...
    function sync_table() {
        ...
        if (assess_session) {
            for (var i = 0; i < assess_session.attributes.length; i++) {
                ...
                var text_table =
                    "<tr>" +
                    '<td><input type="checkbox" id="checkbox_' +
                    i +
                    '" value="' +
                    i +
                    '" name="' +
                ...
                if (attribute.type == "Quantitative") {
                    ...
                    text_table +=
                        ...
                        "</td></tr>";
                    for (
                        var ii = 0, len = attribute.val_med.length;
                        ii < len;
                        ii++
                    ) {
                        text_table +=
                            "<tr><td>" + attribute.val_med[ii] + "</td></tr>";
                    }
                    text_table +=
                        "<tr><td>" + attribute.val_max + "</td></tr></table>";
                }
                ...
                text_table +=
                    ...
                    attribute.mode +
                    "</td>" +
                    '<td><button type="button" id="edit_' +
                    i +
                    '" class="btn btn-outline-dark btn-xs">Edit</button></td>' +
                    '<td><button type="button" class="btn btn-outline-dark" id="deleteK' +
                    i +
                    '"><img src="/static/img/delete.ico" style="width:16px"/></button></td></tr>';
                ...
                (function (_i) {
                    $("#edit_" + _i).click(function () {
                        ...
                        $("#add_attribute h2").text(
                            "Edit attribute " + attribute_edit.name
                        );
                        ...
                        if (attribute_edit.type == "Quantitative") {
                            ...
                            $("#att_value_min_quanti").val(
                                attribute_edit.val_min
                            );
                            $("#att_value_max_quanti").val(
                                attribute_edit.val_max
                            );
                            $(
                                "#att_method_quanti option[value=" +
                                    attribute_edit.method +
                                    "]"
                            ).prop("selected", true);
                            $("#att_mode_quanti").prop(
                                "checked",
                                attribute_edit.mode == "Normal" ? false : true
                            );
                            ...
                            $("#att_value_med_quali_1").val(
                                attribute_edit.val_med[0]
                            );

                            for (
                                var ii = 2, len = attribute_edit.val_med.length;
                                ii < len + 1;
                                ii++
                            ) {
                                var longueur = lists.length,
                                ...
                                new_item.innerHTML =
                                    "<input type='text' class='form-control' id='att_value_med_quali_" +
                                    String(longueur + 1) +
                                    "' placeholder='Value Med " +
                                    String(longueur + 1) +
                                    "'/>";
                                lists[longueur - 1].parentNode.appendChild(
                                    new_item
                                );

                                $("#att_value_med_quali_" + ii).val(
                                    attribute_edit.val_med[ii - 1]
                                );
                            }
    ...
    function verifName(currstring) {
		...
		*/
        ...
        strlist.forEach((element) =>
            testtable.push(accepted.includes(element))
        );
    ...
    $("#submit_quanti").click(function () {
        ...
        if ($("select option:selected").text() == "Probability Equivalence") {
            ...
            $("select option:selected").text() == "Lottery Equivalence"
        ) {
            method = "LE";
        } else if (
            $("select option:selected").text() ==
            "Certainty Equivalence - Constant Probability"
        ) {
            method = "CE_Constant_Prob";
        } else if (
            $("select option:selected").text() ==
            "Certainty Equivalence - Variable Probability"
        ) {
        ...
        if (
            ...
            isThereUnderscore([name, unit], String(val_min), String(val_max)) ==
            false
        ) {
            alert(
                "Please don't write an underscore ( _ ) in your values.\nBut you can put spaces"
            );
        } else if (!verifName(name)) {
            alert(
                "Please use only alphanumerical chars in name (non case sensitive)"
            );
        } else {
            if (edit_mode == false) {
                assess_session.attributes.push({
                    ...
                    val_med: [
                        String(
                            parseFloat(val_min) +
                                0.25 *
                                    (parseFloat(val_max) - parseFloat(val_min))
                        ),
                        String(
                            parseFloat(val_min) +
                                0.5 *
                                    (parseFloat(val_max) - parseFloat(val_min))
                        ), //yes, it is (val_max+val_min)/2, but it looks better ^^
                        String(
                            parseFloat(val_min) +
                                0.75 *
                                    (parseFloat(val_max) - parseFloat(val_min))
                ...
                if (
                    confirm(
                        "Are you sure you want to edit the attribute? All assessements will be deleted"
                    ) == true
                ) {
                            ...
                            String(
                                parseFloat(val_min) +
                                    0.5 *
                                        (parseFloat(val_max) -
                                            parseFloat(val_min))
                            ), //yes, it s (val_max+val_min)/2, but it looks better ^^
                            String(
                                parseFloat(val_min) +
                                    0.75 *
                                        (parseFloat(val_max) -
    ...
    $("#submit_quali").click(function () {
        var name = $("#att_name_quali").val(),
            ...
            nb_med_values = document
                .getElementById("list_med_values_quali")
        ...
        if (name == "" || val_min == "" || val_max == "") {
            ...
            if (edit_mode == false) {
                ...
                if (
                    confirm(
                        "Are you sure you want to edit this attribute? All assessements will be deleted"
                    ) == true
                ) {
                    assess_session.attributes[edited_attribute] = {
                        type: "Qualitative",
