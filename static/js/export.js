...
$(function () {
    ...
    $("#export_xls").click(function () {
        ...
        $.post(
            "ajax",
            '{"type":"export_xlsx", "data":' + data_2_export + "}",
            function (data) {
    ...
    $("#export_xls_option").click(function () {
        ...
        $.post(
            "ajax",
            '{"type":"export_xlsx_option", "data":' + data_2_export + "}",
            function (data) {
                document.location = "export_download/" + data;
            }
        );
    ...
    for (var i = 0; i < assess_session.attributes.length; i++) {
        ...
        (function (_i) {
            ...
            if (points.length > 0 && assess_session.attributes[i].checked) {
                    ...
                    $.post(
                        ...
                        function (data2) {
                            ...
                            for (var key in data) {
                                ...
                                if (key == "exp") {
                                    functions =
                                        '<label style="color:#401539"><input type="checkbox" id="checkbox_' +
                                        _i +
                                        '_exp"> Exponential (' +
                                        Math.round(data[key]["r2"] * 100) /
                                            100 +
                                        ")</label><br/>";
                                    $("#functions_" + _i).append(functions);
                                    data[key]["type"] = "exp";
                                    (function (_data) {
                                        $("#checkbox_" + _i + "_exp").click(
                                            function () {
                                                update_data_export_option(
                                                    _i,
                                                    "exp",
                                                    _data
                                                );
                                    ...
                                    functions =
                                        '<label style="color:#D9585A"><input type="checkbox" id="checkbox_' +
                                        _i +
                                        '_log"> Logarithmic (' +
                                        Math.round(data[key]["r2"] * 100) /
                                            100 +
                                    ...
                                    functions =
                                        '<label style="color:#6DA63C"><input type="checkbox" id="checkbox_' +
                                        _i +
                                        '_pow"> Power (' +
                                        Math.round(data[key]["r2"] * 100) /
                                            100 +
                                    ...
                                    functions =
                                        '<label style="color:#458C8C"><input type="checkbox" id="checkbox_' +
                                        _i +
                                        '_quad"> Quadratic (' +
                                        Math.round(data[key]["r2"] * 100) /
                                            100 +
                                    ...
                                    functions =
                                        '<label style="color:#D9B504"><input type="checkbox" id="checkbox_' +
                                        _i +
                                        '_lin"> Linear (' +
                                        Math.round(data[key]["r2"] * 100) /
                                            100 +
                ...
                $("#functions_" + _i).append(
                    '<input type="checkbox" id="checkbox_' +
                        _i +
                        '_0"> Add just the attribute'
                );
                $("#checkbox_" + _i + "_0").click(function () {
                    update_data_export_option(_i, "0", null);
                });
            }
        })(i);
    }
...
function update_data_export_option(i, type, data) {
    ...
    if (myAttributI != null) {
        //we have an attribut
        if (checked)
            //we are going to add this data in utilities
            data_export_option["attributes"][myAttributI].utilities.push(data);
        //we are goine to remove this type of utilities
        else {
            for (
                var k = 0;
                k <
                data_export_option["attributes"][myAttributI].utilities.length;
                k++
            ) {
                if (
                    data_export_option["attributes"][myAttributI].utilities[k]
                        .type == type
                ) {
                    //we remove it because we are unchecked
                    data_export_option["attributes"][
                        myAttributI
                    ].utilities.splice(k);
                }
            }
            ...
            if (
                data_export_option["attributes"][myAttributI].utilities
                    .length == 0
            )
                data_export_option["attributes"].splice(myAttributI);
        }
    } //we are going to add the good one.
    else {
        var assess_session = JSON.parse(localStorage.getItem("assess_session"));
        myAttribut = assess_session.attributes[i];
        myAttribut.indice = i;
        if (data != null) myAttribut.utilities = [data];
        else myAttribut.utilities = [];
...
function checkbox_multiplicative() {
    ...
    if (checked) {
        ...
        for (var l = 0; l < data_export_option["k_calculus"].length; l++) {
            if (
                data_export_option["k_calculus"][l].method == "multiplicative"
            ) {
