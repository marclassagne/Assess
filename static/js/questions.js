$(function () {
    ...
    for (var i = 0; i < assess_session.attributes.length; i++) {
        if (!assess_session.attributes[i].checked)
            //if this attribute is not activated
        ...
        text_table +=
            '<td><table class="table table-striped" style="width:100%"><tr><td>' +
            attribute.val_min +
            "</td><td> : </td><td>" +
            (attribute.mode == "Normal" ? 0 : 1) +
            "</td></tr>";

        if (attribute.method == "PE" || attribute.method == "LE") {
            for (var ii = 0, len = attribute.val_med.length; ii < len; ii++) {
                text_table +=
                    "<tr><td>" + attribute.val_med[ii] + "</td><td> : </td>";
                if (attribute.questionnaire.points[attribute.val_med[ii]]) {
                    text_table +=
                        "<td>" +
                        attribute.questionnaire.points[attribute.val_med[ii]] +
                        "</td>";
                } else {
                    text_table +=
                        '<td><button type="button" class="btn btn-outline-dark btn-xs answer_quest_' +
                        (attribute.type == "Qualitative" ? "quali" : "quanti") +
                        '" id="q_' +
                        attribute.name +
                        "_" +
                        attribute.val_med[ii] +
                        "_" +
                        ii +
                        '">Assess</button>' +
                        "</td></tr>";
            ...
            for (var key in attribute.questionnaire.points) {
                text_table +=
                    ...
                    "</td><td> : </td>" +
                    "<td>" +
                    attribute.questionnaire.points[key] +
                    "</td></tr>";
            }

            for (
                var ii = Object.keys(attribute.questionnaire.points).length;
                ii < 3;
                ii++
            ) {
                text_table +=
        ...
        text_table +=
            ...
            attribute.val_max +
            "</td><td> : </td><td>" +
            (attribute.mode == "Normal" ? 1 : 0) +
            "</td></tr></table></td>";

        if (attribute.type == "Quantitative") {
            if (attribute.questionnaire.number > 0) {
                if (
                    attribute.questionnaire.number === attribute.val_med.length
                ) {
                    text_table +=
                        '<td><button type="button" class="btn btn-outline-dark btn-xs calc_util_quanti" id="u_' +
                        attribute.name +
            ...
            text_table +=
                '<button type="button" class="btn btn-outline-dark btn-xs" id="excel' +
                i +
            ...
            text_table +=
                '<td><button type="button" class="btn btn-outline-dark btn-xs" id="excel_' +
                i +
                '">export to Excel</button></td>';
        }

        text_table +=
            '<td><button type="button" id="deleteK' +
            i +
            '" class="btn btn-outline-dark btn-xs">Reset</button></td>';
        $("#table_attributes").append(text_table);
        (function (_i) {
            $("#deleteK" + _i).click(function () {
                if (
                    confirm(
                        "Are you sure you want to delete all the assessments for " +
                            assess_session.attributes[_i].name +
                            "?"
                    ) == false
                ) {
                    return;
            ...
            $("#excel" + _i).click(function () {
                ...
                $.post(
                    "ajax",
                    '{"type":"demande_de_transformation", "data":' +
                        big_data +
                        ', "numero":' +
                        _i +
                        "}",
    ...
    $(".answer_quest_quanti").click(function () {
        ...
        if (method == "PE") {
            (function () {
                ...
                arbre_pe.questions_val_max =
                    (mode == "Normal" ? val_max : val_min) + " " + unit;
                arbre_pe.questions_val_min =
                    (mode == "Normal" ? val_min : val_max) + " " + unit;
                arbre_pe.questions_val_mean = gain_certain + " " + unit;

                arbre_pe.display();
                arbre_pe.update();
                $("#trees").append(
                    '</div><div class=choice style="text-align: center;"><p>Which option do you prefer?</p><button type="button" class="btn btn-outline-dark" id="gain">Certain gain</button><button type="button" class="btn btn-outline-dark" id="lottery">Lottery</button></div>'
                );
                // FUNCTIONS
                ...
                function treat_answer(data) {
                    ...
                    if (max_interval - min_interval <= 0.05) {
                        ...
                        ask_final_value(
                            Math.round(
                                ((max_interval + min_interval) * 100) / 2
                            ) / 100
                        );
                    } else {
                        sync_values();
                ...
                function ask_final_value(val) {
                    ...
                    $(".choice").hide();
                    $("#page-wrapper").append(
                        '<div id= "final_value" style="text-align: center;"><br /><br /><p>We are almost done. Please enter the probability that makes you indifferent between the two situations above. Your previous choices indicate that it should be between ' +
                            min_interval +
                            " and " +
                            max_interval +
                            " but you are not constrained to that range <br /> " +
                            min_interval +
                            '\
						 <= <input type="text" class="form-control" id="final_proba" placeholder="Probability" value="' +
                            val +
                    ...
                    $(".final_validation").click(function () {
                        ...
                        if (final_proba <= 1 && final_proba >= 0) {
                            ...
                            assess_session.attributes[
                                indice
                            ].questionnaire.points[String(gain_certain)] =
                                final_proba;
                            assess_session.attributes[
                                indice
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            (function () {
                ...
                // INTERFACE
                var arbre_le = new Arbre(
                    "gauche",
                    "#trees",
                    settings.display,
                    "LE_left"
                );
                var arbre_droite = new Arbre(
                    "droite",
                    "#trees",
                    settings.display,
                    "LE_right"
                ...
                arbre_droite.questions_val_max =
                    parseFloat(question_val) + " " + unit;
                arbre_droite.questions_val_min = val_min + " " + unit;
                arbre_droite.display();
                arbre_droite.update();
                // we add the choice button
                $("#trees").append(
                    '<div class=choice style="text-align: center;"><p>Which option do you prefer?</p><button type="button" class="btn btn-outline-dark lottery_a">Lottery A</button><button type="button" class="btn btn-outline-dark lottery_b">Lottery B</button></div>'
                );
                function treat_answer(data) {
                    min_interval = data.interval[0];
                    max_interval = data.interval[1];
                ...
                function ask_final_value(val) {
                    ...
                    $(".final_validation").click(function () {
                        ...
                        if (final_proba <= 1 && final_proba >= 0) {
                            ...
                            assess_session.attributes[
                                indice
                            ].questionnaire.points[question_val] =
                                final_proba * 2;
                            assess_session.attributes[
                                indice
                            ].questionnaire.number += 1;
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            (function () {
                // VARIABLES
                var min_interval =
                        assess_session.attributes[indice].questionnaire
                            .number == 2
                            ? parseFloat(
                                  Object.keys(
                                      assess_session.attributes[indice]
                ...
                // we add the choice button
                $("#trees").append(
                    '<div class=choice style="text-align: center;"><p>Which option do you prefer?</p><button type="button" class="btn btn-outline-dark" id="gain">Certain gain</button><button type="button" class="btn btn-outline-dark" id="lottery">Lottery</button></div>'
                );
                function utility_finder(gain) {
                    ...
                    if (gain == val_min) {
                        ...
                        for (var key in assess_session.attributes[indice]
                            .questionnaire.points) {
                            if (gain == key) {
                                return assess_session.attributes[indice]
                                    .questionnaire.points[key];
                            }
                        }
                    }
                }
                function treat_answer(data) {
                    ...
                    if (
                        max_interval - min_interval <=
                            0.05 *
                                (parseFloat(arbre_ce.questions_val_max) -
                                    parseFloat(arbre_ce.questions_val_min)) ||
                        max_interval - min_interval < 2
                    ) {
                        //interval <= 5% de l interval de départ ou interval < 2
                        $(".choice").hide();
                ...
                function ask_final_value(val) {
                    ...
                    $(".lottery_b").hide();
                    $("#page-wrapper").append(
                        '<div id= "final_value" style="text-align: center;"><br /><br /><p><p>We are almost done. Please enter the value that makes you indifferent between the two situations above. Your previous choices indicate that it should be between ' +
                            min_interval +
                            " and " +
                            max_interval +
                            " but you are not constrained to that range <br /> " +
                    ...
                    $(".final_validation").click(function () {
                        ...
                        var final_utility =
                            arbre_ce.questions_proba_haut *
                                utility_finder(
                                    parseFloat(arbre_ce.questions_val_max)
                                ) +
                            (1 - arbre_ce.questions_proba_haut) *
                                utility_finder(
                                    parseFloat(arbre_ce.questions_val_min)
                                );
                        ...
                        if (
                            final_gain <=
                                parseFloat(arbre_ce.questions_val_max) &&
                            final_gain >= parseFloat(arbre_ce.questions_val_min)
                        ) {
                            // we save it
                            assess_session.attributes[
                                indice
                            ].questionnaire.points[String(final_gain)] =
                                parseFloat(final_utility);
                            var point_cepv =
                                Object.keys(
                                    assess_session.attributes[indice]
                                        .questionnaire.points
                ...
                $("#lottery").click(function () {
                    $.post(
                        "ajax",
                        '{"type":"question", "method": "CE_Constant_Prob", "gain": ' +
                            String(gain) +
                            ', "min_interval": ' +
                            min_interval +
                            ', "max_interval": ' +
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            (function () {
                ...
                if (
                    ...
                    assess_session.attributes[indice].mode == "Normal"
                ) {
                    p = 0.75;
                } else if (
                    Object.keys(
                ...
                function treat_answer(data) {
                    ...
                    if (
                        max_interval - min_interval <=
                            0.05 *
                                (parseFloat(arbre_cepv.questions_val_max) -
                ...
                function ask_final_value(val) {
                    ...
                    console.log(
                        assess_session.attributes[indice].questionnaire.number
                    );
                }
                // HANDLE USERS ACTIONS
                $("#lottery").click(function () {
    ...
    $(".answer_quest_quali").click(function () {
        ...
        if (method == "PE") {
            (function () {
                ...
                arbre_pe.update();
                $("#trees").append(
                    '</div><div class=choice style="text-align: center;">' +
                ...
                function ask_final_value(val) {
                    ...
                    $("#page-wrapper").append(
                        '<div id= "final_value" style="text-align: center;"><br /><br />' +
                            "<p>We are almost done. Please enter the probability that makes you indifferent between the two situations above. Your previous choices indicate that it should be between " +
                            min_interval +
                            " and " +
                            max_interval +
                            " but you are not constrained to that range <br /> " +
                    ...
                    $(".final_validation").click(function () {
                        ...
                        if (final_proba <= 1 && final_proba >= 0) {
                            ...
                            assess_session.attributes[
                                indice
                            ].questionnaire.points[question_val] = final_proba;
                            assess_session.attributes[
                                indice
                            ].questionnaire.number += 1;
    ...
    $(".calc_util_quanti").click(function () {
        ...
        function calculerAvecArrondi(number, arrondi) {
            ...
            return (
                Math.round(number * Math.pow(10, arrondi)) /
                Math.pow(10, arrondi)
            );
        }
        ...
        function reduce_signe(nombre, dpl = true, signe = true) {
            ...
            if (nombre >= 0 && signe == true) {
                if (dpl == false) {
                    ...
                    return (
                        "+" + calculerAvecArrondi(nombre, settings.decimals_dpl)
                    );
                }
            } else {
                if (dpl == false) {
        ...
        function addTextForm(div_function, copie, render, key, excel) {
            ...
            var copy_button_dpl = $(
                '<button class="btn btn-outline-dark functions_text_form" id="btn_dpl_' +
                    key +
                    '" data-clipboard-text="' +
                    copie +
                    '" title="Click to copy me.">Copy to clipboard (DPL format)</button>'
            );
            var copy_button_excel = $(
                '<button class="btn btn-outline-dark functions_text_form" id="btn_excel_' +
                    key +
                    '" data-clipboard-text="' +
                    excel +
                    '" title="Click to copy me.">Copy to clipboard (Excel format)</button>'
            );
            var copy_button_latex = $(
                '<button class="btn btn-outline-dark functions_text_form" id="btn_latex_' +
                    key +
                    '" data-clipboard-text="' +
                    render +
                    '" title="Click to copy me.">Copy to clipboard (LaTeX format)</button>'
            );
            if (settings.language == "french") {
                ...
                if (mini < 0) {
                    if (key == "exp") {
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#401539">Exponential</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                            reduce_signe(data[i][key]["a"]) +
                            "*exp(" +
                            reduce_signe(-data[i][key]["b"]) +
                            "(" +
                            name +
                            "+" +
                            delta +
                            "))" +
                            reduce_signe(data[i][key]["c"]);
                        var render =
                            reduce_signe(data[i][key]["a"], false, false) +
                            "\\cdot e^{" +
                            reduce_signe(-data[i][key]["b"], false) +
                            "(" +
                            name +
                            "+" +
                            delta +
                            ")}" +
                            reduce_signe(data[i][key]["c"], false);
                        var excel =
                            reduce_signe(data[i][key]["a"]) +
                            "*EXP(" +
                            reduce_signe(-data[i][key]["b"]) +
                        ...
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#D9585A">Logarithmic</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                            ...
                            reduce_signe(data[i][key]["c"]) +
                            ")" +
                            reduce_signe(data[i][key]["d"]);
                        var render =
                            reduce_signe(data[i][key]["a"], false, false) +
                            "\\cdot \\log(" +
                            reduce_signe(data[i][key]["b"], false, false) +
                            "(" +
                            name +
                            "+" +
                            delta +
                            ...
                            reduce_signe(data[i][key]["c"], false) +
                            ")" +
                            reduce_signe(data[i][key]["d"], false);
                        var excel =
                            reduce_signe(data[i][key]["a"]) +
                            "*LN(" +
                            reduce_signe(data[i][key]["b"]) +
                        ...
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#6DA63C">Power</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                        ...
                        var render =
                            ...
                            delta +
                            ")^{" +
                            reduce_signe(1 - data[i][key]["b"], false) +
                            "}-1}{" +
                            reduce_signe(1 - data[i][key]["b"], false) +
                            "}" +
                            reduce_signe(data[i][key]["c"], false);
                        ...
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#458C8C">Quadratic</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                            ...
                            reduce_signe(data[i][key]["a"]);
                        var render =
                            reduce_signe(data[i][key]["c"], false, false) +
                            "\\cdot (" +
                            name +
                            "+" +
                            delta +
                            ...
                            delta +
                            ")^{2}" +
                            reduce_signe(data[i][key]["a"], false);
                        var excel =
                            reduce_signe(data[i][key]["c"]) +
                            "*(" +
                            name +
                        ...
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#D9B504">Linear</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                        ...
                        var div_function = $(
                            '<div id="' +
                                key +
                                '" class="functions_graph" style="overflow-x: auto;"><h3 style="color:#26C4EC">Expo-Power</h3><br />Coefficient of determination: ' +
                                Math.round(data[i][key]["r2"] * 100) / 100 +
                                "<br /><br/></div>"
                        );
                        var copie =
                    ...
                    if (key == "exp") {
                        ...
                        var render =
                            ...
                            reduce_signe(data[i][key]["b"], false, false) +
                            "\\cdot " +
                            assess_session["attributes"][0]["name"] +
                            reduce_signe(data[i][key]["c"], false) +
                            ")" +
                            reduce_signe(data[i][key]["d"], false);
                    ...
                    if (key != "quad") {
                        if (key != "expo-power") {
                            text =
                                ...
                                key +
                                ": " +
                                Math.round(data[key]["r2"] * 10000) / 10000 +
                                ", ";
                        }
                    }
                }
        ...
        $.post("ajax", JSON.stringify(json_2_send), function (data) {
            ...
            var assess_session = JSON.parse(
                localStorage.getItem("assess_session")
            );

            $("#attribute_name").append(
                "<h2>" + assess_session.attributes[indice].name + "</h2>"
            );
            ...
            for (var i = 0; i < LISTE.length; i++) {
                $("#NEWcurves_choice").append(
                    '<tr><td><input type="radio" class="ice" name="select2" value=' +
                        LISTE[i] +
                        "></td><td>" +
                        LISTE[i] +
                        "</td><tr>"
            ...
            $("#charts").append(
                '<table id="curves_choice" class="table"><thead><tr><th></th><th>Points used</th><th>Available regressions: r2</th></tr></thead></table>'
            );
            if (data["data"][0]["quad"] == undefined) {
                for (var i = 0; i < data["data"].length; i++) {
                    regressions_text = availableRegressions2(data["data"][i]);
                    $("#curves_choice").append(
                        '<tr><td><input type="radio" class="hoice" name="select" value=' +
                            i +
                            "></td><td>" +
                            data["data"][i]["points"] +
                            "</td><td>" +
            ...
            $("#checkbox_curves_choice").append(
                '<tr><td><input type="checkbox" class="check_log" id="check_log" name="check_log"></td><td>' +
                    LISTE[0] +
                    "</td><tr>"
            );
            $("#checkbox_curves_choice").append(
                '<tr><td><input type="checkbox" class="check_exp" id="check_exp" name="check_exp"></td><td>' +
                    LISTE[1] +
                    "</td><tr>"
            );
            $("#checkbox_curves_choice").append(
                '<tr><td><input type="checkbox" class="check_pow" id="check_pow" name="check_pow"></td><td>' +
                    LISTE[2] +
                    "</td><tr>"
            );
            $("#checkbox_curves_choice").append(
                '<tr><td><input type="checkbox" class="check_lin" id="check_lin" name="check_lin"></td><td>' +
                    LISTE[3] +
                    "</td><tr>"
            );
            $("#checkbox_curves_choice").append(
                '<tr><td><input type="checkbox" class="check_expopow" id="check_expopow" name="check_expopow"></td><td>' +
                    LISTE[4] +
                    "</td><tr>"
            );
            if (LISTE.length == 6) {
                $("#checkbox_curves_choice").append(
                    '<tr><td><input type="checkbox" class="check_quad" id="check_quad" name="check_quad"></td><td>' +
                        LISTE[5] +
                        "</td><tr>"
                );
            }
    ...
    $(".calc_util_quali").click(function () {
        ...
        $("#charts_quali").show();
        $("#charts_quali").append(
            '<table id="curves_choice_quali" class="table"><thead><tr><th>Choices</th></tr></thead>'
        );
        $("#curves_choice_quali").append(
            '<tbody><tr><td style="text-align:center"><input class="Table_choice" type="radio" name="choice"  value="table">Table</td>'
        );
        ...
        function table_choice() {
            ...
            $("#data_show").append(
                '<table   style="  width:100%;"> <thead><tr><th>value</th><th>utility</th></tr></thead><tbody id="table_info"></tbody>'
            );
